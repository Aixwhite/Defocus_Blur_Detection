...
def train(epo_num=1):
    ...
    for epo in range(epo_num):
        ...
        if np.mod(epo, 10) == 0:
            # 只存储模型参数
            torch.save(enet_model.state_dict(), 'D:\\learning\\moshi\\Defocus-blur-detection-main\\code\\Baselines\\ENet\\models\\enet_model_{}.pth'.format(epo))
            stict = 'D:\\learning\\moshi\\Defocus-blur-detection-main\\code\\Baselines\\ENet\\models\\enet_model_{}.pth'.format(epo)
            print(stict)
            print('saveing ./enet_model_{}.pth'.format(epo))

        if np.mod(epo, 10) == 0:

            test.test(stict,enet_model, "D:\\learning\\moshi\Defocus-blur-detection-main\\code\\Baselines\\ENET\\models_result", test_dataloader)
            mae1, fmeasure1, _, _ = test.eval1("D:\\learning\\moshi\Defocus-blur-detection-main\\code\\Baselines\\ENET\\models_result", "D:\\learning\\moshi\\Defocus-blur-detection-main\\dataset\\test_data\\DUT\\dut500-gt", 0.5)
            print('====================================================================================================================')
