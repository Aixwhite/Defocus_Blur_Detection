...
def train(data_path, trainxml, models_path, backend, snapshot, crop_x, crop_y, batch_size, alpha, epochs, start_lr,
    ...
    traindata = dataloder.BagDataset(dataloder.transform, 'D:\\learning\\moshi\\Defocus-blur-detection-main\\dataset\\train_data\\1204source')
    testdata = dataloder.BagDataset(dataloder.transform, 'D:\\learning\\moshi\\Defocus-blur-detection-main\\dataset\\test_data\\DUT\\dut500-source')
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y,idx) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
        test(net, "D:\\learning\\moshi\Defocus-blur-detection-main\\code\\Baselines\\PsPNet\\models_result", test_loader)
        if np.mod(epoch, 5) == 0:
            torch.save(net.state_dict(), os.path.join(models_path, str(epoch) + ".pth"))
            mae1, fmeasure1, recall1, precesion1 = eval1("D:\\learning\\moshi\Defocus-blur-detection-main\\code\\Baselines\\PsPNet\\models_result", 'D:\\learning\\moshi\\Defocus-blur-detection-main\\dataset\\test_data\\DUT\\dut500-gt', 0.5)
            print("mae:", mae1, "fmeasure:", fmeasure1,"recall:",recall1,"precesion:",precesion1)
    
    #mae1, fmeasure1, _, _ = eval1("./models_result", '."D:\\learning\\moshi\\Defocus-blur-detection-main\\dataset\\test_data"', 1.5)
    print(
        '====================================================================================================================')
